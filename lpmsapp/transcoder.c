...
#define lpms_err( msg, errval, label) {\
  char errstr[AV_ERROR_MAX_STRING_SIZE] = {0}; \
  if (!ret) ret = AVERROR(errval); \
  if (ret <-1) av_strerror(ret, errstr, sizeof errstr); \
  fprintf(stderr, "%s: %s: %s\n", __FUNCTION__, msg, errstr); \
  goto label; \
}


#define main_err(msg, label) { \
  char errstr[AV_ERROR_MAX_STRING_SIZE] = {0}; \
...
*/
  ...
  if (!inp) lpms_err("transcoder: Missing input params\n",123, transcode_cleanup)
  ...
  if (!ictx->ic) {
    ...
    if (ret < 0) main_err("Unable to reopen demuxer", transcode_cleanup);
    ret = avformat_find_stream_info(ictx->ic, NULL);
    if (ret < 0) main_err("Unable to find info for reopened stream", transcode_cleanup)
  } else if (!ictx->ic->pb) {
    // reopen input segment file IO context if needed
    ret = avio_open(&ictx->ic->pb, inp->fname, AVIO_FLAG_READ);
    if (ret < 0) main_err("Unable to reopen file", transcode_cleanup);
  ...
  if (reopen_decoders) {
    ...
    if (AV_HWDEVICE_TYPE_CUDA != ictx->hw_type) {
      ...
      if (ret < 0) main_err("Unable to reopen video decoder", transcode_cleanup);
    }
    ret = open_audio_decoder(inp, ictx);
    if (ret < 0) main_err("Unable to reopen audio decoder", transcode_cleanup)
  }
  ...
  for (i = 0; i <  nb_outputs; i++) {
      ...
      if (!h->initialized || AV_HWDEVICE_TYPE_NONE == octx->hw_type) {
        ...
        if (ret < 0) main_err("transcoder: Unable to open output", transcode_cleanup);
        continue;
      }

      // non-first segment of a HW session
      ret = reopen_output(octx, ictx);
      if (ret < 0) main_err("transcoder: Unable to re-open output for HW session", transcode_cleanup);
  }
...
whileloop_end:
  ...
  for (i = 0; i < nb_outputs; i++) {
    ...
    if (ret < 0) main_err("transcoder: Unable to fully flush outputs", transcode_cleanup)
